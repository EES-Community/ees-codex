...
try {
    ...
    $profileAutoloadBitsBefore = if ($config.ees_architecture -eq '64-bit') {
        if (-not (Test-Path -LiteralPath $config.profile_path -PathType Leaf)) {
            throw "The 64-bit EES preference file was not found: $($config.profile_path)"
        }
        $profileBytes = [System.IO.File]::ReadAllBytes($config.profile_path)
        if ($profileBytes.Length -le 0x38) {
            throw "The 64-bit EES preference file is too short to inspect its library-autoload setting: $($config.profile_path)"
        }
        ([int]$profileBytes[0x38] -band 0x1F)
    }
    else { $null }
    ...
    if ($config.ees_architecture -eq '64-bit') {
        ...
        $profileAutoloadBitsAfter = ([int]$profileBytesAfter[0x38] -band 0x1F)
        if ($profileAutoloadBitsAfter -ne $profileAutoloadBitsBefore) {
            throw 'The /AI launch unexpectedly changed the persistent EES.PRF64 library-autoload bits.'
        }
        $profileRoot = Split-Path -Parent $config.profile_path
        if ((Test-Path -LiteralPath (Join-Path $profileRoot 'EES.PRF64.ees-codex-backup')) -or
            (Test-Path -LiteralPath (Join-Path $profileRoot 'EES.PRF64.ees-codex-backup.json'))) {
    ...
    [ordered]@{
        ...
        profile_autoload_bits_unchanged = if ($config.ees_architecture -eq '64-bit') { $true } else { $null }
    } | ConvertTo-Json
    exit 0
}
catch {
