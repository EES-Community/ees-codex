...
function Enter-EesRunLock {
    ...
    if ([System.IO.Path]::GetFileName($ResolvedEesPath) -ne 'ees64.exe') { return $null }

    $mutex = New-Object System.Threading.Mutex($false, 'Local\EES_Codex_Launcher_64bit_Run')
    $lockTaken = $false
    try {
        try { $lockTaken = $mutex.WaitOne([TimeSpan]::FromSeconds(30)) }
        catch [System.Threading.AbandonedMutexException] { $lockTaken = $true }
        if (-not $lockTaken) {
            throw 'Another 64-bit EES Codex run is active. Wait for it to finish and try again.'
        }

        $otherEes = @(Get-Process -Name 'ees64' -ErrorAction SilentlyContinue)
        if ($otherEes.Count -gt 0) {
            throw "64-bit EES is already running (process ID(s): $($otherEes.Id -join ', ')). Close it before starting a Codex-controlled run."
        }
